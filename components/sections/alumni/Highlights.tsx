    ...
    <section className="bg-surface py-10 sm:py-16">
      <Container className="grid gap-6 grid-cols-1 lg:grid-cols-4">
        {HIGHLIGHTS.map(({ title, icon: Icon, color }) => (
          <article
            key={title}
            className="group relative flex h-full flex-col gap-6 overflow-hidden rounded-none border border-[rgba(0,0,0,0.08)] bg-white px-5 py-5 text-[#414141] transition-transform duration-300 hover:-translate-y-0.5 sm:px-9 sm:py-4"
          >
            <div className="relative flex flex-col gap-6">
              <span
                className={`inline-flex h-12 w-12 items-center justify-center ${color}`}
              >
