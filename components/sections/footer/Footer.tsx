    ...
    <footer id="footer" className="bg-[#060807] pb-14 pt-20 text-[#d6dcd7] sm:pb-20 sm:pt-24">
      <Container className="flex flex-col gap-20">
        <div className="flex flex-col gap-16 lg:flex-row lg:justify-between">
          <div className="flex flex-col gap-6">
            <Link href="#hero" className="text-[2.6rem] font-semibold tracking-[-0.03em] text-white">
              Catalog<span className="ml-2 text-[#7a867c]">↗</span>
            </Link>
            <nav className="flex flex-wrap items-center gap-3 text-base text-[#7a867c]">
              {NAV_LINKS.map((item, index) => (
                <span key={item.label} className="flex items-center gap-3">
                  <Link href={item.href} className="transition-colors hover:text-white">
                    {item.label}
                  </Link>
                  {index !== NAV_LINKS.length - 1 && <span className="text-[#1f221f]">/</span>}
                </span>
              ))}
            </nav>
          </div>

          <section className="grid gap-10 text-sm text-[#9aa49e] sm:grid-cols-2 lg:grid-cols-4 lg:text-base">
            <div className="space-y-2">
              <p className="text-xs uppercase tracking-[0.28em] text-[#2a2f2b]">Contact Us</p>
              <Link href="tel:+18919891191" className="text-xl font-semibold tracking-[-0.02em] text-white">
                +1 891 989-11-91
              </Link>
            </div>
            <div className="space-y-2">
              <p className="text-xs uppercase tracking-[0.28em] text-[#2a2f2b]">Location</p>
              <p>2972 Westheimer Rd. Santa Ana, Illinois 85486</p>
            </div>
            <div className="space-y-2">
              <p className="text-xs uppercase tracking-[0.28em] text-[#2a2f2b]">Email</p>
              <Link href="mailto:hello@iqmento.com" className="transition-colors hover:text-white">
                hello@iqmento.com
              </Link>
            </div>
            <div className="space-y-2">
              <p className="text-xs uppercase tracking-[0.28em] text-[#2a2f2b]">Mo—Fr</p>
        ...
        <div className="flex flex-col gap-12">
          <p className="text-sm text-[#303530]">© {new Date().getFullYear()} — Copyright</p>
            ...
            <div className="relative flex w-full max-w-[1060px] items-center overflow-hidden rounded-[24px]">
              ...
              <div className="absolute inset-0 flex items-center justify-between px-10">
                <div className="flex flex-col gap-1 text-left text-white">
                  <span className="text-base font-medium tracking-[-0.01em]">Explore</span>
                  <span className="text-base font-medium tracking-[-0.01em]">our success</span>
                </div>
